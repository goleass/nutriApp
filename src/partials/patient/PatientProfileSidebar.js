  ...
  return (
    <div className="flex flex-nowrap overflow-x-scroll no-scrollbar md:block md:overflow-auto px-3 py-6 border-b md:border-b-0 md:border-r border-gray-200 min-w-60 md:space-y-3">
          ...
          <li className="mr-0.5 md:mr-0 md:mb-0.5">
            <div className="flex border-gray-200">
              <button className="btn bg-green-500 hover:bg-green-600 text-white flex-1" onClick={(e) => { e.stopPropagation(); setFeedbackModalOpen(true); }}>
                <span className="" aria-controls="feedback-modal" >Adicionar</span>
              </button>
            </div>
          </li>
        </ul>
      ...
      <div>
        <ul className="flex flex-nowrap md:block mr-3 md:mr-0">
          <li className="mr-0.5 md:mr-0 md:mb-0.5">
            <NavLink exact to={`/patients/edit/${id}`} className={`flex items-center px-2.5 py-2 rounded whitespace-nowrap ${pathname.endsWith(`/patients/edit/${id}`) && 'bg-indigo-50'}`}>
          ...
          <li className="mr-0.5 md:mr-0 md:mb-0.5">
            <NavLink exact to={`/patients/edit/${id}/profile`} className={`flex items-center px-2.5 py-2 rounded whitespace-nowrap ${pathname.endsWith(`/patients/edit/${id}/profile`) && 'bg-indigo-50'}`}>
          ...
          <div className="">
            {/* Start */}
            <ModalBasic id="feedback-modal" modalOpen={feedbackModalOpen} setModalOpen={setFeedbackModalOpen} title="O que quer fazer?">
              {/* Modal content */}
      ...
      {/* <div>
        ...
        <ul className="flex flex-nowrap md:block mr-3 md:mr-0">
          <li className="mr-0.5 md:mr-0 md:mb-0.5">
            <NavLink exact to="/profile/account" className={`flex items-center px-2.5 py-2 rounded whitespace-nowrap ${pathname.includes('/profile/account') && 'bg-indigo-50'}`}>
